...
public class AESEncryptor {
    ...
    public byte[] cipher(byte[] in, byte[] key) {
        ...
        byte[][] blocks = new byte[in.length / 16 + 1][16];
    ...
    private byte[][] addRoundKey(byte[][] state, byte[][] roundKey, int roundNumber) {
    ...
    private byte[] xorWord(byte[] a, byte[] b) {
    ...
    private byte[][] byteArrToState(byte[] input){
        byte[][] state = new byte[4][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
